...
export async function GET() {
  ...
  try {
    ...
    return Response.json({ members: result.results.map((row) => ({
      ...
      songStatsAllowed: Number(row.song_stats_allowed) === 1,
    })) });
  } catch (error) {
    return apiError("archive.access.list", error, "회원 권한을 불러오지 못했습니다.", 503);
  }
}
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    return apiError("archive.access.update", error, "회원 권한을 변경하지 못했습니다.");
  }
}
