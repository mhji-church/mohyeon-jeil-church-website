...
export async function PATCH(request: Request) {
  ...
  if (body?.action === "resolve-conflict" && body.id) {
    ...
    catch (error) { return Response.json({ error: error instanceof Error ? error.message : "충돌 기록을 처리하지 못했습니다." }, { status: 400 }); }
  }
  ...
  catch (error) { return Response.json({ error: error instanceof Error ? error.message : "찬양을 수정하지 못했습니다." }, { status: 400 }); }
}
