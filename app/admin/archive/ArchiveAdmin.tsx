...
type MemberLoadState = "idle" | "loading" | "ready" | "error";
...
export default function ArchiveAdmin({ userName, userEmail, signOutPath, mode = "manage" }: Props) {
  ...
  const loadMembers = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error ?? "회원 등급을 불러오지 못했습니다.");
      setMembers(data.members ?? []);
      setMemberLoadState("ready");
    } catch (error) {
      setMemberLoadError(error instanceof Error ? error.message : "회원 등급을 불러오지 못했습니다.");
  ...
  useEffect(() => {
    if (!formDirty && !settingsDirty) return;
  ...
  useEffect(() => {
    ...
    const syncTabFromUrl = () => {
      ...
      setTab(requested === "access" || requested === "settings" ? requested : "videos");
      setForm(emptyForm);
      setFormDirty(false);
    };
  ...
  function update<K extends keyof FormState>(key: K, value: FormState[K]) { setForm((current) => ({ ...current, [key]: value })); setFormDirty(true); }
  function chooseSongSuggestion(title: string) { setForm((current) => { const lines = current.songsText.split(/\r?\n/); lines[lines.length - 1] = title; return { ...current, songsText: lines.join("\n") }; }); setFormDirty(true); setSongSuggestions([]); }

  function confirmDiscardChanges() {
    return (!formDirty && !settingsDirty) || window.confirm("저장하지 않은 변경이 있습니다. 페이지를 이동할까요?");
  }
  ...
  function handleAdminNavigate(key: ArchiveNavKey, href: string) {
    if (mode !== "manage" || (key !== "videos" && key !== "members" && key !== "settings")) return false;
    const nextTab = key === "members" ? "access" : key === "settings" ? "settings" : "videos";
    setTab(nextTab);
    setForm(emptyForm);
    setFormDirty(false);
  ...
  function selectManageTab(nextTab: "videos" | "access") {
    if (!confirmDiscardChanges()) return;
    handleAdminNavigate(nextTab === "access" ? "members" : "videos", nextTab === "access" ? "/archive/admin?tab=access" : "/archive/admin");
  }
  ...
  async function changeSongStatsAccess(member: AccessMember, songStatsAllowed: boolean) {
    if (updatingMemberId || songStatsAllowed === member.songStatsAllowed) return;
    setUpdatingMemberId(member.id);
    try {
      const response = await fetch("/api/admin/archive/access", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ memberId: member.id, songStatsAllowed }) });
      if (!response.ok) throw new Error("찬양 통계 권한을 변경하지 못했습니다.");
      await loadMembers();
      setNotice({ message: `${member.name}님의 찬양 통계 권한을 변경했습니다.`, tone: "success" });
    } catch (error) {
      setNotice({ message: error instanceof Error ? error.message : "찬양 통계 권한을 변경하지 못했습니다.", tone: "error" });
    } finally {
      setUpdatingMemberId("");
    }
  }
  ...
  async function saveSettings(event: React.FormEvent) {
    ...
    try { const response = await fetch("/api/archive/settings", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify(settings) }); const data = await response.json(); if (!response.ok) throw new Error(data.error); setSettings(data.settings); setSettingsDirty(false); setNotice({ message: "아카이브 운영 설정을 저장했습니다.", tone: "success" }); }
  ...
  const videoForm = (variant: "page" | "drawer") => (
    <form className={`archive-admin-form archive-editor-form admin-card admin-form is-${variant}`} onSubmit={save}>
      <div className="archive-form-heading">
        ...
        {variant === "drawer" && <button className="archive-drawer-close" type="button" onClick={closeEditor} aria-label="영상 수정 닫기">×</button>}
      </div>
      ...
      <div className="archive-admin-form-actions button-row">{form.id && <button className="secondary-btn" type="button" onClick={closeEditor}>수정 취소</button>}<button className="primary-btn" type="submit" disabled={saving}>{saving ? "저장 중" : form.id ? "수정 저장" : "영상 등록"}</button></div>
    </form>
  );

  return (
    <ArchiveShell admin active={tab === "access" ? "members" : tab === "settings" ? "settings" : "videos"} beforeAdminNavigate={confirmDiscardChanges} onAdminNavigate={handleAdminNavigate} account={<a className="header-action-link" href={signOutPath} aria-label="로그아웃"><ArchiveIcon name="logout" size={17} className="mobile-header-action-icon" /><span>로그아웃</span></a>}>
      <section className={`archive-admin-workspace${mode === "new" ? " is-new-video" : " is-list-mode"}`}>
        ...
        {mode === "new" ? <div className="archive-form-page">{videoForm("page")}</div> : tab === "videos" ? <>
          ...
          {form.id && <div className="archive-edit-layer"><button className="archive-edit-backdrop" type="button" aria-label="영상 수정 닫기" onClick={closeEditor} /><aside className="archive-edit-drawer" role="dialog" aria-modal="true" aria-label="영상 수정">{videoForm("drawer")}</aside></div>}
        </> : tab === "access" ? <section className="admin-card archive-access-list" aria-busy={memberLoadState === "loading"}><div className="admin-list-heading"><h2>회원별 열람 등급</h2><span>{memberLoadState === "ready" ? `총 ${members.length}명` : "회원 정보를 확인합니다"}</span></div>{memberLoadState === "loading" ? <div className="archive-access-feedback" role="status"><span className="cms-nav-spinner" aria-hidden="true" />회원 목록을 불러오는 중…</div> : memberLoadState === "error" ? <div className="archive-access-feedback is-error" role="alert"><strong>회원 목록을 불러오지 못했습니다.</strong><span>{memberLoadError}</span><button className="secondary-btn" type="button" onClick={() => void loadMembers()}>다시 시도</button></div> : <div className="table-wrap"><table className="admin-table"><thead><tr><th>회원</th><th>계정 상태</th><th>아카이브 등급</th><th>찬양 통계</th><th>열람 범위</th></tr></thead><tbody>{members.map((member) => <tr key={member.id}><td><strong>{member.name}</strong><small>{member.username}</small></td><td>{member.status}</td><td><select value={member.accessLevel} disabled={updatingMemberId === member.id} onChange={(event) => void changeAccess(member, event.target.value as ArchiveAccessLevel)}><option value="none">권한 없음</option><option value="worship">예배 영상</option><option value="full">전체 기록</option></select></td><td><select aria-label={`${member.name} 찬양 통계 권한`} value={member.songStatsAllowed ? "allowed" : "denied"} disabled={updatingMemberId === member.id || member.accessLevel === "none"} onChange={(event) => void changeSongStatsAccess(member, event.target.value === "allowed")}><option value="allowed">허용</option><option value="denied">차단</option></select></td><td>{updatingMemberId === member.id ? "저장 중…" : accessLabels[member.accessLevel]}</td></tr>)}</tbody></table>{members.length === 0 && <div className="archive-list-empty">표시할 회원이 없습니다.</div>}</div>}</section> : <div className="archive-settings-grid"><form className="admin-card archive-settings-panel" onSubmit={saveSettings}><div className="admin-list-heading"><h2>운영 설정</h2><span>공개 화면·등록 기본값</span></div><div className="archive-settings-fields"><label><span>홈 최근 영상 수</span><select value={settings.recentCount} onChange={(event) => { setSettings((current) => ({ ...current, recentCount: Number(event.target.value) as 4 | 8 | 12 })); setSettingsDirty(true); }}><option value="4">4개</option><option value="8">8개</option><option value="12">12개</option></select></label><label><span>기본 정렬</span><select value={settings.defaultSort} onChange={(event) => { setSettings((current) => ({ ...current, defaultSort: event.target.value as Settings["defaultSort"] })); setSettingsDirty(true); }}><option value="newest">최신순</option><option value="oldest">오래된순</option></select></label><label><span>신규 영상 기본 예배</span><select value={settings.defaultServiceType} onChange={(event) => { setSettings((current) => ({ ...current, defaultServiceType: event.target.value })); setSettingsDirty(true); }}><option>주일 1부 예배</option><option>주일 2부 예배</option><option>수요예배</option><option>특별예배</option></select></label><label><span>기본 설교자</span><input value={settings.defaultPreacher} onChange={(event) => { setSettings((current) => ({ ...current, defaultPreacher: event.target.value })); setSettingsDirty(true); }} /></label><label className="archive-setting-check"><input type="checkbox" checked={settings.autoInspectYoutube} onChange={(event) => { setSettings((current) => ({ ...current, autoInspectYoutube: event.target.checked })); setSettingsDirty(true); }} /><span>URL 입력 후 자동으로 영상 정보 확인</span></label><label><span>등록 완료 후</span><select value={settings.afterSave} onChange={(event) => { setSettings((current) => ({ ...current, afterSave: event.target.value as Settings["afterSave"] })); setSettingsDirty(true); }}><option value="list">목록으로 이동</option><option value="continue">계속 등록</option></select></label></div><div className="archive-settings-actions"><button className="primary-btn" disabled={savingSettings}>{savingSettings ? "저장 중…" : "설정 저장"}</button></div></form><section className="admin-card archive-system-status"><div className="admin-list-heading"><h2>시스템 상태</h2><span>비밀값은 표시하지 않습니다</span></div><div className="archive-status-cards"><article><small>데이터베이스</small><strong>{systemStatus?.database === "connected" ? "정상 연결" : "확인 중"}</strong></article><article><small>YouTube API</small><strong>{systemStatus ? systemStatus.youtubeApi ? "설정됨" : "미설정" : "확인 중"}</strong></article><article><small>전체 기록</small><strong>{systemStatus?.total ?? "-"}개</strong></article><article><small>예배 / 출석</small><strong>{systemStatus ? `${systemStatus.worship} / ${systemStatus.attendance}` : "-"}</strong></article></div><div className="archive-security-summary"><h3>권한 정책</h3><p>회원 데이터는 홈페이지와 공유하지만 관리자 세션은 분리됩니다. 홈페이지 관리자 쿠키로는 이 화면과 아카이브 관리 API에 접근할 수 없습니다.</p><p>마지막 영상 갱신: {systemStatus?.latest ? new Date(systemStatus.latest).toLocaleString("ko-KR") : "기록 없음"}</p></div></section></div>}
      </section>
    </ArchiveShell>
  );
}
