...
export default function SongStats({ viewerName, viewerKind }: { viewerName: string; viewerKind: "admin" | "member" }) {
  ...
  const [start, setStart] = useState(""); const [end, setEnd] = useState(""); const [limit, setLimit] = useState("50"); const [query, setQuery] = useState("");
  ...
  return <ArchiveShell active="songs" account={<Link className="header-action-link user-link" href={viewerKind === "admin" ? "/archive/admin" : "/member"}>{viewerName}</Link>}>
    <section className="song-stats-page">
      ...
      <div className="song-stats-filters">
        ...
        <label className="song-stats-search"><span>찬양 검색</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="대표 제목·별칭 검색" /></label>
