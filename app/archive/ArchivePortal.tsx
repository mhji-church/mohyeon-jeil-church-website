...
export default function ArchivePortal({ initialAccess }: { initialAccess: AccessState }) {
  ...
  function renderCard(video: ArchiveVideo, featuredCard = false) {
    ...
    return <article key={video.id} className={`media-card${video.type === "worship" ? " media-card--worship" : ""}${featuredCard ? " featured" : ""}`}>
      ...
      {!featuredCard && <div className="media-meta"><span className={`media-date${duplicateWorshipMetadata ? " is-title-duplicate" : ""}`}>{formatDate(video.date)} · {video.serviceType}</span><h3 aria-label={video.title} title={video.title}><button className="media-title-button" type="button" onClick={(event) => void play(video, event.currentTarget)} aria-label={`${video.title} ${allowed ? "재생" : "로그인 후 시청"}`} title={video.title}>{video.type === "attendance" ? <><span className="attendance-title-desktop">{video.title}</span><span className="attendance-title-mobile">{formatAttendanceTitle(video.date)}</span></> : video.title}</button></h3>{video.type !== "attendance" && <p>설교 · {video.preacher || "모현제일교회"}</p>}</div>}
    </article>;
  }
