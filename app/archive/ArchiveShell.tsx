...
export function ArchiveShell({ children, active, admin = false, search, account }: { children: ReactNode; active: ArchiveNavKey; admin?: boolean; search?: ReactNode; account?: ReactNode }) {
  ...
  return <div className={`archive-original-root${admin ? " admin-cms" : ""}`} data-theme="light">
    ...
    {!admin && <nav className="mobile-bottom-nav" aria-label="모바일 예배 아카이브 메뉴">{publicNav.map(([key, label, href, icon]) => <Link className={active === key ? "active" : ""} href={href} key={key}><ArchiveIcon name={icon} size={22} /><span>{label}</span></Link>)}</nav>}
  </div>;
}
