...
test("mobile gallery header stays reachable and selecting photos does not move the reader", async ({ page, browserName }, testInfo) => {
  ...
  for (const width of [320, 390]) {
    ...
    const fullscreen = dialog.getByRole("button", { name: "사진 전체화면 보기" });
    const controlsBox = await dialog.locator(".gallery-zoomable .zoom-controls").boundingBox();
    expect(controlsBox.height).toBeLessThanOrEqual(42);
    expect(controlsBox.width).toBeLessThan(width - 12);
    ...
    const focusBox = await stage.boundingBox();
    expect(focusBox.width).toBeGreaterThanOrEqual(width - 1);
    expect(focusBox.height).toBeGreaterThanOrEqual(719);
    await dialog.getByRole("button", { name: "확대", exact: true }).click();
    await expect(dialog.locator("output")).toHaveText("125%");
    await dialog.getByRole("button", { name: "다음 사진", exact: true }).click();
    await expect(dialog.locator(".gallery-photo-focus-count")).toHaveText(/^[12] \/ 2$/);
    await dialog.getByRole("button", { name: "사진 전체화면 종료" }).click();
    await expect(dialog.getByRole("button", { name: "사진 전체화면 보기" })).toBeFocused();
    expect(Math.abs(await reader.evaluate(el => el.scrollTop) - scrollBeforeFocus)).toBeLessThan(2);

    await dialog.getByRole("button", { name: "사진 전체화면 보기" }).click();
    await page.keyboard.press("Escape");
    await expect(dialog.getByRole("button", { name: "사진 전체화면 보기" })).toBeFocused();
    await stage.evaluate(element => Object.defineProperty(element, "requestFullscreen", { configurable: true, value: undefined }));
    await dialog.getByRole("button", { name: "사진 전체화면 보기" }).click();
    expect(await page.evaluate(() => document.fullscreenElement)).toBeNull();
    await expect(dialog).toHaveClass(/is-photo-focus/);
    expect(await page.evaluate(() => window.history.state?.mhjiGalleryPhotoFocus)).toBe(true);
    await page.evaluate(() => window.history.back());
    await expect.poll(() => page.evaluate(() => Boolean(window.history.state?.mhjiGalleryPhotoFocus))).toBe(false);
    await expect(dialog).toBeVisible();
    await expect(dialog.getByRole("button", { name: "사진 전체화면 보기" })).toBeFocused();
